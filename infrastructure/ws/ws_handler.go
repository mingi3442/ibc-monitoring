			...
			if eventBlockData, ok := event.Data.(types.EventDataNewBlock); ok {
				*recentState = int64(eventBlockData.Block.Height)
				...
				if !found {
					...
					*recentState = int64(eventTxData.TxResult.Height)
					// ! Transaction이 발생하지 않으니까... 이 부분은 실행되지 않는 것 같음
					logger.Debug("message.action in Transaction: %X", types.Tx(eventTxData.Tx).Hash())
